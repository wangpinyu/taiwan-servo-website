# 電動缸 optimization tracking

Branch: `phase-1-smac-spec-standard`
Priority: 1
Pages: 14
Blocking pages: 13

## Status counts

- agent-fix-required: 12
- agent-approved-clean: 1
- agent-structure-review: 1

## Target pages

| ID | Title | Status | Preview |
| --- | --- | --- | --- |
| 100 | Thomson 電動缸 | agent-fix-required | preview/products/detail/100.html |
| 167 | SMAC 線性致動器 | agent-approved-clean | preview/products/detail/167.html |
| 168 | Servomech 電動缸 | agent-fix-required | preview/products/detail/168.html |
| 170 | Thomson升降柱(Lifting Columns) | agent-fix-required | preview/products/detail/170.html |
| 171 | Thomson不銹鋼食品級線性致動器 | agent-fix-required | preview/products/detail/171.html |
| 172 | Duff-Norton 線性致動器千斤頂接頭 | agent-fix-required | preview/products/detail/172.html |
| 173 | CMCO 升降器 | agent-structure-review | preview/products/detail/173.html |
| 373 | SMAC圓形音圈致動器 | agent-fix-required | preview/products/detail/373.html |
| 374 | SMAC線性加旋轉致動器 | agent-fix-required | preview/products/detail/374.html |
| 375 | SMAC線性滑塊致動器 | agent-fix-required | preview/products/detail/375.html |
| 376 | Thomson智慧型重載電動推桿 | agent-fix-required | preview/products/detail/376.html |
| 377 | Thomson通用工業電動推桿 | agent-fix-required | preview/products/detail/377.html |
| 378 | Thomson精密與特殊應用電動推桿 | agent-fix-required | preview/products/detail/378.html |
| 379 | Thomson軌道式電動推桿 | agent-fix-required | preview/products/detail/379.html |

## Acceptance criteria

- npm run validate passes.
- Target pages are agent-approved-clean, or source-needed is justified.
- Product spec details sit under Product Series.
- Tables, accordions, CTAs, downloads, and SEO checks match project rules.
- No public-facing internal notes, local paths, placeholder links, or .txt hrefs.

